...
@Component({
  selector: 'app-production-bovin',
  templateUrl: './production-bovin.component.html',
  styleUrls: ['./production-bovin.component.css']
})
export class ProductionBovinComponent implements OnInit {
  ...
  loadData(): any {
    ...
    const marketCulture = this.angularFireDatabase.database.ref().child('Bovin')
  ...
  selectCedar(arg) {
    const array = [];
    const exploitant = this.angularFireDatabase.database.ref().child('exploitants');
    const cedar = this.angularFireDatabase.database.ref().child('Bovin');
