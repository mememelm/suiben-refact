...
@Component({
  selector: 'app-production-cunniculture',
  templateUrl: './production-cunniculture.component.html',
  styleUrls: ['./production-cunniculture.component.css']
})
export class ProductionCunnicultureComponent implements OnInit {
  ...
  loadData(): any {
    ...
    const marketCulture = this.angularFireDatabase.database.ref().child('Cunniculture')
  ...
  selectCedar(arg) {
    const array = [];
    const exploitant = this.angularFireDatabase.database.ref().child('exploitants');
    const cedar = this.angularFireDatabase.database.ref().child('Cunniculture');
