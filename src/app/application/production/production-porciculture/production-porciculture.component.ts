...
@Component({
  selector: 'app-production-porciculture',
  templateUrl: './production-porciculture.component.html',
  styleUrls: ['./production-porciculture.component.css']
})
export class ProductionPorcicultureComponent implements OnInit {
  ...
  loadData(): any {
    ...
    const marketCulture = this.angularFireDatabase.database.ref().child('Poriculture')
  ...
  selectCedar(arg) {
    const array = [];
    const exploitant = this.angularFireDatabase.database.ref().child('exploitants');
    const cedar = this.angularFireDatabase.database.ref().child('Poriculture');
