...
export class CuviComponent implements OnInit {
  ...
  getFoodCulture(): any {
    ...
    const marketCulture = this.angularFireDatabase.database.ref().child('FoodCulture')
  ...
  selectCedar(arg) {
    const array = [];
    const exploitant = this.angularFireDatabase.database.ref().child('exploitants');
    const cedar = this.angularFireDatabase.database.ref().child('FoodCulture');
  ...
  openModal(arg: any) {
    ...
    data.push(dataPush)
    localStorage.setItem('foodProductionByExploitant', JSON.stringify(dataPush))
  }
