...
@Component({
  selector: 'app-production-aviculture',
  templateUrl: './production-aviculture.component.html',
  styleUrls: ['./production-aviculture.component.css']
})
export class ProductionAvicultureComponent implements OnInit {
  ...
  loadData(): any {
    ...
    const marketCulture = this.angularFireDatabase.database.ref().child('Aviculture')
  ...
  selectCedar(arg) {
    const array = [];
    const exploitant = this.angularFireDatabase.database.ref().child('exploitants');
    const cedar = this.angularFireDatabase.database.ref().child('Aviculture');
